...
* Introduction
  ...
  client software for the [[http://www.vmware.com/products/vcenter-orchestrator/][VMware vCenter Orchestrator]], and more precisely for
  its SOAP WebService. This site provides:
...
* Client libraries
  ...
  (since they share the same developer), so this one might be the easiest
  starting point for running examples.
* Simulator versions
